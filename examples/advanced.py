...
def main():
    instance = Deepplanner(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced deepplanner Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.decompose(input=inp, index=i) for i, inp in enumerate(inputs)]
    ...
    start = time.time()
    for i in range(1000):
        instance.decompose(iteration=i)
